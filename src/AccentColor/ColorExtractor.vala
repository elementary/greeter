...
public class Greeter.ColorExtractor : Object {
    private const double PERCENTAGE_SAMPLE_PIXELS = 0.01;

    public Gdk.Pixbuf? pixbuf { get; construct set; }
    public Greeter.Color? primary_color { get; construct set; }

    private Gee.List<Greeter.Color> pixels;
    ...
    public ColorExtractor.from_primary_color (Greeter.Color primary_color) {
        Object (primary_color: primary_color);

        pixels = new Gee.ArrayList<Greeter.Color> ();
        pixels.add (primary_color);
    }

    public int get_dominant_color_index (Gee.List<Greeter.Color> palette) {
        int index = 0;
    ...
    private Gee.ArrayList<Greeter.Color> convert_pixels_to_rgb (uint8[] pixels, bool has_alpha) {
        var list = new Gee.ArrayList<Greeter.Color> ();
        ...
        for (int i = 0; i < pixels.length / factor; i += step_size) {
            ...
            list.add (new Greeter.Color (red, green, blue, 0.0));
        }
